...
std::vector<double> readAudio(const std::string& path) {
    AVFormatContext* formatCtx = nullptr;
    if (avformat_open_input(&formatCtx, path.c_str(), nullptr, nullptr) != 0) {
        throw DhtError("Cannot open audio file: " + path);
    }
    ...
    if (audioStreamIdx == -1) {
        avformat_close_input(&formatCtx);
        throw DhtError("No audio stream found");
    }
    ...
    if (!codec) {
        avformat_close_input(&formatCtx);
        throw DhtError("Codec not found");
    }

    AVCodecContext* codecCtx = avcodec_alloc_context3(codec);
    if (!codecCtx) {
        avformat_close_input(&formatCtx);
        throw DhtError("Failed to allocate codec context");
    }

    if (avcodec_parameters_to_context(codecCtx, codecParams) < 0) {
        avcodec_free_context(&codecCtx);
        avformat_close_input(&formatCtx);
        throw DhtError("Failed to copy codec params");
    }

    if (avcodec_open2(codecCtx, codec, nullptr) < 0) {
        avcodec_free_context(&codecCtx);
        avformat_close_input(&formatCtx);
        throw DhtError("Failed to open codec");
    }
